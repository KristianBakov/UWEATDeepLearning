...
if not os.path.isfile(training_binary_path):
  ...
  training_data = []
  faces_path = os.path.join(DATA_PATH,'augmentedf+')
  for filename in tqdm(os.listdir(faces_path)):
      ...
      image = Image.open(path).resize((GENERATE_SQUARE,GENERATE_SQUARE),Image.ANTIALIAS).convert('RGB')
      #image = np.expand_dims((image),0)
      #gen_iter = gen.flow(image)
      #aug_images = [next(gen_iter)[0].astype(np.uint8) for i in range(10)]
...
generator.save(os.path.join("./saves","brick_generator.h5"))
discriminator.save(os.path.join("./saves","brick_discriminator.h5"))
